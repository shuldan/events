...
fmt: $(GOIMPORTS)
	...
	@$(GOIMPORTS) -local github.com/shuldan/events -w $$(find . -name "*.go" -not -path "./vendor/*")
...
fmt-check: $(GOIMPORTS)
	...
	@unformatted_imports=$$($(GOIMPORTS) -local github.com/shuldan/events -l . | grep -v vendor || true); \
